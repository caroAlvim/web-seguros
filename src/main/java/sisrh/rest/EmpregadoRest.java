...
public class EmpregadoRest {
	...
	@Path("status/{status}")
	@Produces(MediaType.APPLICATION_JSON)
	public Response listarEmpregadosStatus(@PathParam("status") int status) throws Exception {
		try {
		
		List<Empregado> lista = Banco.listarEmpregadosPorStatus(status);		
		GenericEntity<List<Empregado>> entity = new GenericEntity<List<Empregado>>(lista) {};
		return Response.ok().entity(entity).build();}
		catch (Exception e) {
			return Response.status(Status.INTERNAL_SERVER_ERROR)
					.entity("{ \"mensagem\" : \"Falha ao obter os empregados, verifique o status!\" , \"detalhe\" :  \""+ e.getMessage() +"\"  }").build();
		}
	}
	...
	public Response incluirEmpregado(Empregado empregado) {
		try {
			...
			return Response.status(Status.INTERNAL_SERVER_ERROR).entity("{ \"mensagem\" : \"Falha na inclusao do empregado!\" , \"detalhe\" :  \""+ e.getMessage() +"\"  }").build();
		}		
	}
